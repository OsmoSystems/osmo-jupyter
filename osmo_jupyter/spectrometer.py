        ...
        for line in infile:
            if line.strip() == SPECTRAL_DATA_IDENTIFIER:
                unformatted_spectrometer_df = pd.read_csv(
                    io.StringIO(infile.read()), sep="\t", lineterminator="\n"
                )

                return unformatted_spectrometer_df
        raise ValueError(
            "Spectrometer data not in expected format; check the file header"
        )
...
def import_and_format_spectrometer_data(spectrometer_data_file_path):
    ...
    spectrometer_df_with_clean_header = _clean_up_spectrometer_df_header(
        unformatted_spectrometer_df
    )
    reformatted_spectrometer_df = _reformat_spectrometer_df(
        spectrometer_df_with_clean_header
    )
...
def spectrometer_intensity_summary(
    reformatted_spectrometer_df,
    ...
    relevant_wavelengths = reformatted_spectrometer_df["wavelength"].between(
        minimum_wavelength, maximum_wavelength
    )
    ...
    intensity_summary = (
        filtered_spectrometer_data.groupby(["timestamp"])
        .mean()
