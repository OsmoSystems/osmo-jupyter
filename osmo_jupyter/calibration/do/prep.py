...
def prep_calibration_data(
    ysi_data_filepath, image_data_filepath, do_patch_roi_name, reference_patch_roi_name
):
    ...
    """
    # import YSI data
    ysi_data = pd.read_csv(ysi_data_filepath, parse_dates=["Timestamp"]).set_index(
        "Timestamp"
    )
    ...
    # Import camera data
    all_camera_data = pd.read_csv(image_data_filepath, parse_dates=["timestamp"])

    # Pull out some choice MSORMs
    r_msorms = (
        all_camera_data.set_index(["timestamp", "ROI"])
        .unstack()["r_msorm"]
        .reset_index()
    )

    # Join with YSI data and prep for calibration
    desired_ysi_columns = ["Temperature (C)", "Dissolved Oxygen (mmHg)"]
    msorms_and_ysi_data = join_interpolated_ysi_data(
        r_msorms, ysi_data[desired_ysi_columns]
    )
    ...
    calibration_data_set = msorms_and_ysi_data.rename(
        calibration_data_rename, axis="columns"
    )[list(calibration_data_rename.values())]
    calibration_data_set["SR reading"] = (
        calibration_data_set["DO patch reading"]
        / calibration_data_set["Reference patch reading"]
    )
