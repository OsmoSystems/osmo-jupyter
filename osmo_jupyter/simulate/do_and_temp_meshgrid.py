...
DO_MESHGRID, TEMPERATURE_MESHGRID = np.meshgrid(
    DO_DOMAIN, TEMPERATURE_DOMAIN, indexing="ij"
)
...
def get_meshgrid_of_do_and_temp(fn_of_do_and_temp):
    ...
    return np.array(
        [
            [fn_of_do_and_temp(do, temperature) for temperature in TEMPERATURE_DOMAIN]
            for do in DO_DOMAIN
        ]
    )


def plot_meshgrid_of_do_and_temp_as_surface(
    meshgrid_of_do_and_temp, title, z_axis_title
):
    ...
    """
    contour = {"show": True, "width": 1, "project": {"x": True, "y": True, "z": True}}
    ...
    return go.FigureWidget(
        [
            go.Surface(
                ...
                opacity=0.9,
                contours=go.surface.Contours(x=contour, y=contour, z=contour),
            )
        ],
        layout={
            "title": title,
...
def plot_fn_of_do_and_temp_as_surface(fn_of_do_and_temp, title, z_axis_title):
    ...
    return plot_meshgrid_of_do_and_temp_as_surface(
        get_meshgrid_of_do_and_temp(fn_of_do_and_temp), title, z_axis_title
    )
