...
class TestSimulateSpatialRatiometricReading:
    ...
    def test_calls_with_correct_kwargs(self, mock_get_optical_reading_normalized):
        ...
        expected_unsealed_patch_call = call(sentinel.do, sentinel.temperature, **unsealed_patch_kwargs)
        expected_sealed_patch_call = call(sentinel.sealed_patch_do, sentinel.temperature, **sealed_patch_kwargs)

        mock_get_optical_reading_normalized.assert_has_calls([
            expected_unsealed_patch_call,
            expected_sealed_patch_call
        ])

    def test_returns_ratio_of_unsealed_to_sealed(self, mock_get_optical_reading_normalized):
        ...
        assert actual == (mock_optical_reading_for_unsealed_patch / mock_optical_reading_for_sealed_patch)
