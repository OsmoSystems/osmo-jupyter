...
class TestNormalizedReading:
    @pytest.mark.parametrize(
        ...
        [
            (
                "max fluorescence at low DO, low temp",
                DO_MIN_MMHG,
                TEMPERATURE_STANDARD_OPERATING_MIN,
                1,
            ),
            (
                "min fluorescence at high DO, high temp",
                DO_MAX_MMHG,
                TEMPERATURE_STANDARD_OPERATING_MAX,
                0,
            ),
    ...
    def test_normalized_reading_bounds(
        self, name, do, temperature, expected_normalized_reading
    ):
        assert (
            module.get_optical_reading_normalized(do, temperature)
            == expected_normalized_reading
        )
...
class TestEstimateOpticalReading:
    ...
    def test_estimate_optical_reading(
